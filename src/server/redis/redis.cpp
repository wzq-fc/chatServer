...
bool Redis::connect()
{
    ...
    if(_publish_context==nullptr)
    {
        cerr<<"与redis服务器简历连接失败"<<endl;
        return false;
    }
    _subcribe_context=redisConnect("127.0.0.1",6379);
    if(_subcribe_context==nullptr)
    {
        cerr<<"与redis服务器建立连接失败"<<endl;
        return false;
    }
...
bool Redis::subcribe(int channel)
{
    ...
    if(redisAppendCommand(this->_subcribe_context,"SUBCRIBE %d",channel)==REDIS_ERR)
    {
        cerr<<"订阅失败"<<endl;
        return false;
    }
    ...
    while(!done)
    {
        if(redisBufferWrite(this->_subcribe_context,&done))
        {
            cerr<<"订阅失败"<<endl;
            return false;
        }
...
{
    if(redisAppendCommand(this->_subcribe_context,"UNSUBCRIBE %d",channel)==REDIS_ERR)
    {
        cerr<<"取消订阅失败"<<endl;
        return false;
    }
